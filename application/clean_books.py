...
def clean_books(path: str, date_cols: list = [], loan_date_col: str = None, return_date_col: str = None):
    # Load csv file into dataframe
    books_raw = pd.read_csv(path)

    # Drop items with blank Id value
    books_raw = books_raw.dropna(subset=['Id'])
...
def save_to_sql(df, name, schema):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dates', nargs='*', type=str, help='(Optional) Specify date columns in CSV file.')
    parser.add_argument('--loan_date_col', type=str, help="(Optional) Specify column that indicates when book was loaned out. If also specifying return_date_col, then calculate DaysLoaned column.")
    parser.add_argument('--return_date_col', type=str, help="(Optional) Specify column that indicates when book was returned. If also specifying loan_date_col, then calculate DaysLoaned column.")

    args = parser.parse_args()

    books, drop = clean_books(
        path=args.rel_path,
        date_cols=args.dates,
    ...
    save_to_sql(books, 'Books', 'Library')
